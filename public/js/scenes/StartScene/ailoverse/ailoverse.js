export function ailoverseLevel0(self, Moralis) {
...
export function ailoverseLevel1(self, Moralis) {
    // MAKE GROUP FOR LEVEL
    self.levelGroup = self.add.group();
...
export function ailoverseLevel2(self, Moralis) {

    // MAKE GROUP FOR LEVEL
    self.levelGroup = self.add.group();

    self.cats = self.add.image(500, 450, 'ailoverse-cats').setScale(0.2);
    self.robots = self.add.image(600, 600, 'ailoverse-robots').setScale(0.2);
    self.ailoverseText = self.add.text(500, 100, 'AILOVERSE', { fill: "#ffffff", fontSize: "48px", fontFamily: "PixelFont" });
