export function witchesLevel0(self, Moralis) {
...
export function witchesLevel1(self, Moralis) {
    // MAKE GROUP FOR LEVEL
    self.levelGroup = self.add.group();
...
export function witchesLevel2(self, Moralis) {
    ...
    // TEXT
    var text = 'TO ENTER THE ROOM YOU SHOULD HAVE CRYPTO COVEN NFT';
    self.label = self.add.text(250, 200, '', { fill: "#ffffff", fontSize: "24px", align: "center" });
    self.levelGroup.add(self.label);
    self.typeTextWithDelay(text);
    ...
    }).layout().setPosition(600, 300).setAlpha(0);

    // SET BUTTONS INTERCTIVE
    self.button1.setInteractive().on('pointerdown', () => {
        if (self.step != 2) return;
        self.witch.setAlpha(0);
        self.progress.setAlpha(1);
        self.button1.setAlpha(0);
        self.label.setPosition(500, 480);
        self.label.text = 'CHECKING YOUR NFT...';
        const token_address = "0x5180db8f5c931aae63c74266b211f580155ecac8";
